...
const PACKET_TIMEOUT_MS: u128 = 500;
...
fn main() {
    ...
    let programmer_address = programmer_hello_event.programmer_address;

    println!("Connected to programmer ({:#04x})", programmer_address);

    loop {
        if let Ok(packet) = try_get_packet(&mut port, &mut builder) {
            println!("{:?}", packet);
        }
    }
}
