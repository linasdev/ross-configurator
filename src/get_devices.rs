...
pub fn get_devices(protocol: &mut Protocol<Serial>, programmer: &ProgrammerHelloEvent) -> Result<Vec<BootloaderHelloEvent>, ConfiguratorError>  {
    let mut devices: Vec<BootloaderHelloEvent> = match protocol.exchange_packets(programmer.to_packet(), false, TRANSACTION_RETRY_COUNT as u32, || {
